...
def test_bresenham2D():
  ...
  x,y = bresenham2D(800, 800, 787, 1102)
  plt.imshow(np.zeros([1601,1601]))
  plt.scatter(x,y)
  plt.savefig('bresenham_test.jpg')
...
def test_mapCorrelation():
    ...
    xs0_w, ys0_w, _ = transformation.lidarToWorld(xs0, ys0, 0, 0, 0)
    xs0_m, ys0_m = transformation.worldToMap(MAP, xs0_w, ys0_w)
    Y = np.stack((xs0_m,ys0_m))
    
    # convert from meters to cells
    xis = np.ceil((xs0_m - MAP['xmin']) / MAP['res'] ).astype(np.int16)-1
    yis = np.ceil((ys0_m - MAP['ymin']) / MAP['res'] ).astype(np.int16)-1
    ...
    x_range = np.arange(-80,80,0.1)
    y_range = np.arange(-80,80,0.1)
    ...
    fig1 = plt.figure()
    plt.imshow(np.ones(MAP['map'].shape) * 255)
    plt.scatter(xs0_m,ys0_m,c='r', marker="s")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.title("Laser reading")
    plt.axis('equal')
    plt.savefig("Laser reading")
    ...
    X, Y = np.meshgrid(np.arange(0,1600), np.arange(0,1600))
    ...
    plt.savefig("Correlation coefficient map")

    plt.show()
  
  
def show_lidar():
  _, lidar_data = read_data_from_csv('Lidar/lidar.csv')
  angles = np.linspace(-5, 185, 286) / 180 * np.pi
  ranges = lidar_data[0, :]
  plt.figure()
